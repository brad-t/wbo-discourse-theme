      ...
      <nav>
        {{#each NAV_ITEMS as |item|}}
          ...
          {{! Expand Community in place with Unread + categories. Tapping
              Community routes to /latest. Unread stays here because
              Discourse's own pill row hides it when count is 0 and never
              shows it off the discovery route — the drawer is the one
              place it's always reachable. Not an accordion — you're
